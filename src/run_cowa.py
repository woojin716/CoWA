"""
CoWA test-time adaptation experiment runner (offline evaluation, cached).

Loads pre-saved testset_*.pt files (generated by save_test_dataset.py with
xrayvision preprocessing already applied) to skip data preprocessing.

Usage:
    python src/run_cowa.py --source chexpert --target mimic --exp_id cowa_test1
    python src/run_cowa.py --source chexpert --target all --exp_id cowa_test1
    python src/run_cowa.py --source chexpert --target all --exp_id cowa_test1 --cache-dir ./data
"""
...
from cowa import get_cowa_adapter
...
def compute_metrics_from_tensors(
    ...
    # Build mapping from canonical labels to model output indices
    label_mapping = {}
...
def run_tta_experiment(
    ...
    **tta_kwargs
    ...
    """Run a single CoWA TTA experiment (offline evaluation)."""
    ...
    adapter = get_cowa_adapter(model, device=device,
                               source_name=source_model, target_name=target_dataset,
                               **tta_kwargs)

    # Offline evaluation: adapt, then predict
...
def append_result_to_csv(result: Dict, output_dir: Path):
    """Append a single experiment's results to the CSV files."""
...
def run_all_experiments(
    ...
    **tta_kwargs
    ...
    """Run all CoWA experiments (offline evaluation, cached)."""
...
def main():
    ...
    # CoWA hyperparameters (default=None means: fall back to config file value)
    ...
    # Pass only non-None values so they override the config file defaults
    _all_kwargs = {
        'lr': args.lr,
        'steps': args.steps,
        'top_k': args.top_k,
